...
1. 绝对路径：从根 / 开始，如 /home/user/file

2. 相对路径：从当前目录开始，. 表示当前目录，.. 表示上级目录
...
1. PATH：冒号分隔的目录列表，Shell 按顺序查找可执行文件。

2. 查看：echo $PATH

3. 临时添加目录：export PATH="/path/to/dir:$PATH"

4. 查看命令位置：which 命令 或 command -v 命令

5. 注意：临时修改只在当前 Shell 有效，永久修改需写入 ~/.bashrc 或 ~/.profile。
